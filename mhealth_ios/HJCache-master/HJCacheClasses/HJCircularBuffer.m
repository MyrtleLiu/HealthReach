...
-(NSInteger)findIndexOfObject:(id)key {
	NSNull* nullObj = [NSNull null];
	id obj;
	NSInteger i = nextIndex-1; //start looking at the last object
	if (i<0) { i=0; }
	for (NSInteger n = [buffer count]-1; n>=0; n--) {
		if (i<0) { 
...
-(id)findObject:(id)key {
	NSInteger i = [self findIndexOfObject:key];
	if (i<0) { 
		return nil;
	}
