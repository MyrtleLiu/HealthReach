...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) 
    {
        ...
        bacground.frame=CGRectMake(0.0, 0.0, frame.size.width, frame.size.height);
...
- (void)determineCurrentRow
{
    ...
    NSInteger position = round(delta / 39.0);
    currentRow = position;
...
- (void)tileViews
{
    ...
    NSInteger firstNeededViewIndex = floorf(CGRectGetMinY(visibleBounds) / 39.0) - 2;
	...
	for (NSInteger index = firstNeededViewIndex; index <= lastNeededViewIndex; index++)
	{
        if (![self isDisplayingViewForIndex:index]) 
		{
			...
			if (label == nil)
            {
				label = [[UILabel alloc] initWithFrame:CGRectMake(_rowIndent, 0, glassImageView.frame.size.width, 39.0)];
