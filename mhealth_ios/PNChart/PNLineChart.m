...
- (void)setYLabels:(NSArray *)yLabels
{
    ...
    if (yStep == 0.0) {
        PNChartLabel *minLabel = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, _chartCavanHeight, _chartMargin, _yLabelTheHeight)];
        minLabel.text = [NSString stringWithFormat:yLabelFormat, 0.0];
        [self addSubview:minLabel];

        PNChartLabel *midLabel = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, _chartCavanHeight/2, _chartMargin, _yLabelTheHeight)];
        midLabel.text = [NSString stringWithFormat:yLabelFormat, _yValueMax];
        [self addSubview:midLabel];

        PNChartLabel *maxLabel = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, 0.0, _chartMargin, _yLabelTheHeight)];
        ...
        while (num > 0) {
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, (_chartCavanHeight - index * yStepHeight), _chartMargin, _yLabelTheHeight)];
...
- (void)strokeChart
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        if (!_showLabel) {
            _chartCavanHeight = self.frame.size.height - 2 * _yLabelTheHeight;
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            int y = _chartCavanHeight - (innerGrade * _chartCavanHeight) + (_yLabelTheHeight / 2);
...
- (void)drawRect:(CGRect)rect
{
    if (self.isShowCoordinateAxis) {
        ...
        if (self.showLabel) {
            ...
            CGFloat yStepHeight = _chartCavanHeight / _yLabelNum;
            for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
                point = CGPointMake(_chartMargin + yAsixOffset, (_chartCavanHeight - i * yStepHeight + _yLabelTheHeight/2));
...
- (void)setDefaultValues
{
    ...
    _yLabelNum = 5.0;
    _yLabelTheHeight = [[[[PNChartLabel alloc] init] font] pointSize];
